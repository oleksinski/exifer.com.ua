...
class SafeHTML
{
    ...
    function _writeAttrs ($attrs)
    {
        if (is_array($attrs)) {
            foreach ($attrs as $name => $value) {
                ...
                if (_strpos($name, 'on') === 0) {
                    continue;
                }
                if (_strpos($name, 'data') === 0) {
                    continue;
                }
                if (in_array($name, $this->attributes)) {
                    continue;
                }
                if (!_preg_match("/^[a-z0-9]+$/i", $name)) {
                ...
                if ($name == 'style') {
                   ...
                   foreach ($this->_cssRegexps as $css) {
                       if (_preg_match($css, $value)) {
                           continue 2;
                       }
                   }
                   foreach ($this->_protoRegexps as $proto) {
                       if (_preg_match($proto, $value)) {
                           continue 2;
                       }
                   }
                }

                $tempval = _preg_replace('/&#(\d+);?/me', "chr('\\1')", $value); //"'
                $tempval = _preg_replace('/&#x([0-9a-f]+);?/mei', "chr(hexdec('\\1'))", $tempval);

                if ((in_array($name, $this->protocolAttributes)) &&
                    (_strpos($tempval, ':') !== false))
                {
                    if ($this->protocolFiltering == 'black') {
                        foreach ($this->_protoRegexps as $proto) {
                            if (_preg_match($proto, $tempval)) continue 2;
                        }
                    } else {
    ...
    function parse($doc)
    {
       ...
       // Save all '<' symbols
       $doc = _preg_replace("/<(?=[^a-zA-Z\/\!\?\%])/", '&lt;', $doc);

       // Web documents shouldn't contains \x00 symbol
       $doc = _str_replace("\x00", '', $doc);

       // Opera6 bug workaround
       $doc = _str_replace("\xC0\xBC", '&lt;', $doc);
    ...
    {
       return _preg_replace_callback('!\+([0-9a-zA-Z/]+)\-!', array($this, 'repackUTF7Callback'), $str);
    }
    ...
    function repackUTF7Callback($str)
    {
       ...
       $str = _preg_replace_callback('/^((?:\x00.)*)((?:[^\x00].)+)/', array($this, 'repackUTF7Back'), $str);
       return _preg_replace('/\x00(.)/', '$1', $str);
    }
    ...
    {
       return $str[1].'+'._rtrim(base64_encode($str[2]), '=').'-';
    }
}
