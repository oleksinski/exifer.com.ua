...
class XML_HTMLSax3_StateParser {
    ...
    */
    function scanUntilString($string) {
        $start = $this->position;
        $this->position = _strpos($this->rawtext, $string, $start);
        if ($this->position === FALSE) {
            $this->position = $this->length;
        }
        return _substr($this->rawtext, $start, $this->position - $start);
    }
...
class XML_HTMLSax3_StateParser_Lt430 extends XML_HTMLSax3_StateParser {
    ...
    */
    function scanUntilCharacters($string) {
        ...
        while ($this->position < $this->length && _strpos($string, $this->rawtext{$this->position}) === FALSE) {
            $this->position++;
        }
        return _substr($this->rawtext, $startpos, $this->position - $startpos);
    }
...
class XML_HTMLSax3_StateParser_Gtet430 extends XML_HTMLSax3_StateParser {
    ...
    */
    function scanUntilCharacters($string) {
        ...
        return _substr($this->rawtext, $startpos, $length);
    }
