...
class XML_HTMLSax3_CaseFolding {
    ...
    */
    function foldOpen(&$parser, $tag, $attrs=array(), $empty = FALSE) {
        $this->orig_obj->{$this->orig_open_method}($parser, _strtoupper($tag), $attrs, $empty);
    }
    /**
    ...
    */
    function foldClose(&$parser, $tag, $empty = FALSE) {
        $this->orig_obj->{$this->orig_close_method}($parser, _strtoupper($tag), $empty);
    }
}
/**
...
class XML_HTMLSax3_Entities_Parsed {
    ...
    */
    function breakData(&$parser, $data) {
        $data = _preg_split('/(&.+?;)/',$data,-1,PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
        foreach ( $data as $chunk ) {
            $chunk = _html_entity_decode($chunk,ENT_NOQUOTES);
            $this->orig_obj->{$this->orig_method}($this, $chunk);
        }
    }
}
    ...
    */
    function strip(&$parser, $data) {
        ...
        if ( _substr($data,0,2) == '--' ) {
            ...
            $data = _preg_replace($patterns,'',$data);

        // Check for XML CDATA sections (note: don't do both!)
        } else if ( _substr($data,0,1) == '[' ) {
            $patterns = array(
