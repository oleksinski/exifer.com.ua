...
class phpmailerTest extends PHPUnit_Framework_TestCase {
	...
	@include './testbootstrap.php'; //Overrides go in here
        ...
        if (array_key_exists('mail_cc', $_REQUEST) and _strlen($_REQUEST['mail_cc']) > 0) {
    ...
    function BuildBody() {
        ...
        if($this->Mail->ContentType == "text/html" || _strlen($this->Mail->AltBody) > 0)
        {
            $eol = "<br/>";
            $bullet = "<li>";
            $bullet_start = "<ul>";
	...
	$t = _substr(file_get_contents(__FILE__), 0, 1024); //Just pick a chunk of this file as test content
    ...
    function test_DenialOfServiceAttack() {
        ...
        $this->Mail->Subject = _str_repeat("A", 998);
        $this->assertTrue($this->Mail->Send(), $this->Mail->ErrorInfo);
    }
	...
	*/
	function test_Translations() {
		...
		foreach (new DirectoryIterator('../language') as $fileInfo) {
			...
			if (_preg_match('/^phpmailer\.lang-([a-z_]{2,})\.php$/', $fileInfo->getFilename(), $matches)) {
	...
	*/
	function test_Encodings() {
	    $this->Mail->Charset = 'iso-8859-1';
	    $this->assertEquals('=A1Hola!_Se=F1or!', $this->Mail->EncodeQ('ЎHola! Seсor!', 'text'), 'Q Encoding (text) failed');
	    $this->assertEquals('=A1Hola!_Se=F1or!', $this->Mail->EncodeQ('ЎHola! Seсor!', 'comment'), 'Q Encoding (comment) failed');
	    $this->assertEquals('=A1Hola!_Se=F1or!', $this->Mail->EncodeQ('ЎHola! Seсor!', 'phrase'), 'Q Encoding (phrase) failed');
	}
...
<p/>
<input type="submit" value="Run Test" class="button" />
